...
export default function AdminPage() {
  ...
  const saveMenuItem = async () => {
    ...
    try {
      if (editMenuItem._id && !editMenuItem.isNew) await API.put(`/menu/${editMenuItem._id}`, editMenuItem);
      else await API.post('/menu', editMenuItem);
      setEditMenuItem(null); loadMenu();
    } catch {}
  };
  const deleteMenuItem = async (id) => {
    if (!window.confirm('Удалить блюдо?')) return;
    try { await API.delete(`/menu/${id}`); loadMenu(); } catch {}
  };
  ...
  useEffect(() => {
    const interval = setInterval(loadShift, 30000);
  ...
  return (
    <div className="page">
      ...
      <div className="tabs">
        ...
        {settings.showIngredients && <button className={`tab${tab==='ingredients'?' active':''}`} onClick={()=>setTab('ingredients')}>Компоненты</button>}
      ...
      {tab==='menu' && (
        <div style={{padding:16}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
            <div className="section-title" style={{margin:0}}>{settings.adminCanEditMenu ? 'Меню' : 'Меню (только просмотр)'}</div>
            {settings.adminCanEditMenu && (
              <button onClick={()=>setEditMenuItem({name:'',category:'',price:0,weight:'',description:'',hasSauce:false,isNew:true})} className="btn btn-gold" style={{padding:'7px 14px',fontSize:13}}>+ Добавить</button>
            )}
          </div>
          {[...new Set(menu.map(i=>i.category))].map(cat=>(
            <div key={cat} style={{marginBottom:16}}>
              ...
              {menu.filter(i=>i.category===cat).map(item=>(
                <div key={item._id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid var(--border)',opacity:item.available?1:0.5}}>
                  <div>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    ...
                    {settings.adminCanEditMenu && (
                      <button onClick={()=>setEditMenuItem({...item})} style={{padding:'4px 10px',borderRadius:6,fontSize:12,background:'var(--bg3)',color:'var(--text2)',border:'1px solid var(--border)'}}>✏</button>
                    )}
                  </div>
                </div>
              ))}
      ...
      {/* INGREDIENTS VIEW - admin can view only */}
      {tab==='ingredients' && settings.showIngredients && (
        <div style={{padding:16}}>
          <div className="section-title">Компоненты блюд (только просмотр)</div>
          {menu.filter(i=>i.ingredients&&i.ingredients.length>0).map(item=>(
            <div key={item._id} style={{background:'var(--bg2)',borderRadius:10,padding:12,marginBottom:10,border:'1px solid var(--border)'}}>
              <div style={{fontWeight:600,fontSize:14,marginBottom:8}}>{item.name}</div>
              {item.ingredients.map((ing,idx)=>(
                <div key={idx} style={{display:'flex',gap:8,fontSize:13,color:'var(--text3)',padding:'3px 0'}}>
                  <span>•</span>
                  <span>{ing.inventoryName}</span>
                  <span style={{color:'var(--gold)'}}>{ing.quantity} {ing.unit}</span>
                </div>
              ))}
            </div>
          ))}
          {menu.filter(i=>i.ingredients&&i.ingredients.length>0).length===0 && (
            <div className="empty">Компоненты не добавлены. Добавьте их в разделе superadmin.</div>
          )}
        </div>
      )}
...
function ShiftManager({ shiftData, users, closeRequest, onConfirmClose, onUpdate, onClose, showToast }) {
  ...
  const canEdit = true; // Admin can always edit shift assignments
  ...
  return (
    <div>
      ...
      {cooks.map(u=>{
        ...
        return(
          <div key={u._id} onClick={()=>canEdit&&toggleCook(u)} className="card" style={{border:`1px solid ${sel?'var(--gold)':'var(--border)'}`,cursor:canEdit?'pointer':'default',background:sel?'rgba(232,200,74,0.06)':'var(--bg2)'}}>
        ...
        return(
          <div key={u._id} className="card" style={{border:`1px solid ${sel?'var(--gold)':'var(--border)'}`}}>
            ...
            {sel ? (
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {['neon','elvis','enot'].map(club=>(
                  <button key={club} onClick={()=>canEdit&&setGarconClub(u,club)} style={{flex:1,padding:'8px',borderRadius:8,fontSize:13,fontWeight:700,background:sel.club===club?`${CC[club]}18`:'var(--bg3)',color:sel.club===club?CC[club]:'var(--text3)',border:`1px solid ${sel.club===club?CC[club]:'var(--border)'}`}}>{club.toUpperCase()}</button>
                ))}
                {canEdit&&<button onClick={()=>removeGarcon(u._id)} style={{padding:'8px 10px',borderRadius:8,background:'rgba(231,76,60,0.1)',color:'var(--red)',border:'1px solid rgba(231,76,60,0.2)',fontSize:13}}>✕</button>}
              </div>
            ) : canEdit && (
      ...
      {isOpen && (
        <button onClick={onConfirmClose} className="btn btn-red" style={{width:'100%',padding:15,fontSize:15,marginTop:16}}>
          🔒 Закрыть смену
        </button>
      )}
