...
export default function KitchenPage() {
  ...
  const printReport = async () => {
    try {
      const r = await API.get('/shift/report');
      await fetch('http://localhost:3001/print-report', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(r.data)
      });
    } catch (e) { console.warn('Report print failed:', e.message); }
  };

  const requestClose = async () => {
    try { await API.put('/shift/close-request'); setCloseModal(false); notify('Запрос на закрытие отправлен'); printReport(); } catch {}
  };
...
function InventoryView({ inventory, onReload, cookName }) {
  ...
  const expected = inventory.expectedConsumption || {};
  ...
  const closeShift = async () => {
    try {
      const r = await API.put(`/inventory/${inventory._id}/close`, { cookNames: [cookName] });
      setDiscrepancies(r.data.discrepancies || []);
  ...
  return (
    <div style={{ padding:16 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 0 8px 0', borderBottom:'1px solid var(--border)', marginBottom:8 }}>
        <div style={{ flex:1, fontSize:11, color:'var(--text3)' }}>Продукт</div>
        <div style={{ width:42, fontSize:11, color:'var(--text3)', textAlign:'center' }}>Вчера</div>
        <div style={{ width:52, fontSize:11, color:'#f39c12', textAlign:'center' }}>Ожид.</div>
        <div style={{ width:76, fontSize:11, color:'var(--text3)', textAlign:'center' }}>Сейчас</div>
        <div style={{ width:38, fontSize:11, color:'var(--text3)', textAlign:'center' }}>Δ</div>
        <div style={{ width:72, fontSize:11, color:'var(--text3)', textAlign:'center' }}>Статус</div>
      </div>
          ...
          {items.filter(i => i.category===cat).map((item, idx) => {
            ...
            return (
              <InvRow key={item._id} item={item} disabled={inventory.shift==='closed'}
                expectedConsumption={expected[item.name]}
                onUpdate={update}
      ...
      {showDiscModal && (
        <div className="modal-overlay" onClick={() => setShowDiscModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxHeight:'80vh', overflowY:'auto' }}>
            <div style={{ fontWeight:700, fontSize:18, marginBottom:4 }}>📊 Расхождения</div>
            <div style={{ fontSize:13, color:'var(--text3)', marginBottom:16 }}>Разница между ожидаемым и фактическим расходом</div>
            {!discrepancies?.length ? (
              <div style={{ textAlign:'center', padding:20, color:'var(--green)', fontSize:15, fontWeight:600 }}>✓ Всё сходится!</div>
            ) : discrepancies.map((d, i) => (
              <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom:'1px solid var(--border)' }}>
                <div>
                  <div style={{ fontSize:14, fontWeight:600 }}>{d.name}</div>
                  <div style={{ fontSize:12, color:'var(--text3)' }}>Ожид.: {d.expected} · Факт: {d.actual}</div>
                </div>
                <div style={{ fontSize:15, fontWeight:700, color: d.diff > 0 ? 'var(--red)' : 'var(--green)' }}>
                  {d.diff > 0 ? `+${d.diff}` : d.diff}
                </div>
              </div>
            ))}
            <button onClick={() => setShowDiscModal(false)} className="btn btn-gold" style={{ width:'100%', marginTop:16 }}>Принято ✓</button>
          </div>
        </div>
      )}
    </div>
...
function InvRow({ item, onUpdate, disabled, inputRef, onEnter, expectedConsumption }) {
  ...
  const c = cons();
  const expNow = () => {
    const y = parseFloat(item.yesterdayValue);
    if (isNaN(y) || expectedConsumption === undefined) return null;
    return (y - expectedConsumption).toFixed(1);
  };
  const exp = expNow();
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 0', borderBottom:'1px solid var(--border)' }}>
      ...
      <div style={{ width:52, fontSize:12, textAlign:'center', color:'#f39c12', fontWeight:600 }}>{exp !== null ? exp : '—'}</div>
