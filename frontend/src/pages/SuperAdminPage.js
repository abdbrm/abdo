...
export default function SuperAdminPage() {
  ...
  const saveIngredients = async () => {
    try {
      await API.put(`/menu/${editIngredients._id}/ingredients`, { ingredients: editIngredients.ingredients });
      setEditIngredients(null); loadAll(); showToast('Компоненты сохранены ✓');
    } catch {}
  };
    ...
    <div className="page">
      ...
      {tab==='visibility' && (
        <div style={{padding:16}}>
          ...
          <Toggle k="showIngredients" label="Компоненты блюд" desc="Управление компонентами блюд у администратора" />
          <Toggle k="adminCanEditMenu" label="Редактирование меню администратором" desc="Разрешить администратору добавлять и удалять блюда" />
            ...
            <div key={cat} style={{marginBottom:18}}>
              ...
              {menu.filter(i=>i.category===cat).map(item=>(
                <div key={item._id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 0',borderBottom:'1px solid var(--border)'}}>
                  ...
                  <button onClick={()=>setEditIngredients({...item,ingredients:item.ingredients||[]})} style={{padding:'6px 10px',borderRadius:6,fontSize:13,background:'rgba(52,152,219,0.1)',color:'#3498db',border:'1px solid rgba(52,152,219,0.2)'}}>🧪</button>
      ...
      {editItem && (
        <div className="modal-overlay" onClick={()=>setEditItem(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {editIngredients && (
        <div className="modal-overlay" onClick={()=>setEditIngredients(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()} style={{maxHeight:'80vh',overflowY:'auto'}}>
            <div style={{fontWeight:700,fontSize:18,marginBottom:4}}>🧪 Компоненты: {editIngredients.name}</div>
            <div style={{fontSize:12,color:'var(--text3)',marginBottom:14}}>Название должно совпадать с названием в запасах</div>
            {(editIngredients.ingredients||[]).map((ing,idx)=>(
              <div key={idx} style={{display:'flex',gap:6,marginBottom:8,alignItems:'center'}}>
                <input placeholder="Название (как в запасах)" value={ing.inventoryName} onChange={e=>setEditIngredients(p=>({...p,ingredients:p.ingredients.map((x,i)=>i===idx?{...x,inventoryName:e.target.value}:x)}))} style={{flex:2,fontSize:13}} />
                <input type="number" placeholder="Кол." value={ing.quantity} onChange={e=>setEditIngredients(p=>({...p,ingredients:p.ingredients.map((x,i)=>i===idx?{...x,quantity:+e.target.value}:x)}))} style={{width:60,fontSize:13}} />
                <input placeholder="ед." value={ing.unit||''} onChange={e=>setEditIngredients(p=>({...p,ingredients:p.ingredients.map((x,i)=>i===idx?{...x,unit:e.target.value}:x)}))} style={{width:48,fontSize:13}} />
                <button onClick={()=>setEditIngredients(p=>({...p,ingredients:p.ingredients.filter((_,i)=>i!==idx)}))} style={{padding:'4px 8px',borderRadius:6,background:'rgba(231,76,60,0.1)',color:'var(--red)',border:'1px solid rgba(231,76,60,0.2)',fontSize:13}}>✕</button>
              </div>
            ))}
            <button onClick={()=>setEditIngredients(p=>({...p,ingredients:[...(p.ingredients||[]),{inventoryName:'',quantity:1,unit:''}]}))} style={{width:'100%',padding:'9px',borderRadius:8,background:'var(--bg3)',border:'1px solid var(--border)',color:'var(--text2)',fontSize:13,marginBottom:14}}>+ Добавить компонент</button>
            <button onClick={saveIngredients} className="btn btn-gold" style={{width:'100%'}}>Сохранить ✓</button>
          </div>
        </div>
      )}
